...
export function IGoalApp() {
  ...
  const scopedActions = useMemo(
    () => role === "IKAMER" ? actions.filter((action) => action.assigneeId === "USR-TOM") : actions,
    [actions, role],
  );
  const visibleActions = useMemo(() => scopedActions.filter((action) => {
    if (actionFilter === "COMPLETED") return action.status === "COMPLETED";
    if (action.status !== "OPEN") return false;
    if (actionFilter === "OVERDUE") return action.dueAt < "2026-07-14";
    return true;
  }), [actionFilter, scopedActions]);
  const visibleRisks = useMemo(() => risks.filter((risk) => {
    if (riskFilter === "RESOLVED") return risk.status === "RESOLVED" || risk.status === "IGNORED";
    if (risk.status === "RESOLVED" || risk.status === "IGNORED") return false;
    return riskFilter === "ACTIVE" || risk.severity === riskFilter;
  }), [riskFilter, risks]);
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Điều hướng chính">
          ...
          <button className={`nav-item ${view === "HOME" ? "active" : ""}`} onClick={() => navigate("HOME")}><Icon>⌂</Icon>Tổng quan</button>
          ...
          <button className={`nav-item ${view === "ACTIONS" ? "active" : ""}`} onClick={() => navigate("ACTIONS")}><Icon>↗</Icon>Action items<span className="nav-count subtle">{actions.filter((action) => action.status === "OPEN").length}</span></button>
          <button className={`nav-item ${view === "RISKS" ? "active" : ""}`} onClick={() => navigate("RISKS")}><Icon>!</Icon>Risk Center<span className="nav-count risk-count">{risks.filter((risk) => risk.status !== "RESOLVED" && risk.status !== "IGNORED").length}</span></button>
